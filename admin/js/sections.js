window.AdminSections = (() => {
  ...
  function updateCountdownOptions(container) {
    ...
    if (options.some(([value]) => value === previous)) countdown.value = previous;
    else if (options.length) countdown.value = options[0][0];
  }

  function setPhotoState(photoField, hasPhoto) {
    photoField.querySelector(".photo-preview-stage").dataset.hasPhoto = String(hasPhoto);
    photoField.querySelector(".photo-select-label").textContent = hasPhoto ? "Ganti Foto" : "Pilih Foto";
  ...
  return {
    render: (sections, wedding) => sections.map((section) => renderers[section]
      ? renderers[section](wedding)
      : (section !== "rsvp" && console.warn(`Section renderer belum tersedia: ${section}`), "")).join(""),

    bind: (container) => {
      container.onclick = (event) => {
        const target = event.target;
        if (target.classList.contains("remove-account")) target.parentElement.remove();
        if (target.classList.contains("add-account")) target.insertAdjacentHTML("beforebegin", `<div class="account-row">${field("Bank", "account_bank")}${field("Nomor", "account_number")}${field("Atas Nama", "account_holder")}<button type="button" class="remove-account">Hapus</button></div>`);
      ...
      container.querySelectorAll(".photo-upload").forEach((input) => {
        input.onchange = () => {
          ...
          if (!file || file.size > 1048576 || !["image/jpeg", "image/png", "image/webp"].includes(file.type)) {
            window.AdminUI?.showToast("Foto harus JPEG, PNG, atau WEBP dengan ukuran maksimum 1 MB.", "error");
            input.value = "";
            return;
          }
      ...
      const akad = container.querySelector('[name="akad_enabled"]');
      const reception = container.querySelector('[name="reception_enabled"]');
      const sameAsAkad = container.querySelector('[name="reception_same_as_akad"]');
      const receptionFields = eventCopyFields.map((key) => container.querySelector(`[name="reception_${key}"]`));
      const akadFields = eventCopyFields.map((key) => container.querySelector(`[name="akad_${key}"]`));
      const copyAkadToReception = () => eventCopyFields.forEach((key, index) => {
        if (akadFields[index] && receptionFields[index]) receptionFields[index].value = akadFields[index].value;
      });
      const updateSameAsAkad = () => {
        ...
        receptionFields.forEach((input) => { if (input) input.disabled = sameAsAkad.checked; });
      };
      akad?.addEventListener("change", () => updateCountdownOptions(container));
      reception?.addEventListener("change", () => updateCountdownOptions(container));
      ...
      akadFields.forEach((input) => input?.addEventListener("input", () => {
        if (sameAsAkad?.checked) copyAkadToReception();
      }));
      akadFields.forEach((input) => input?.addEventListener("change", () => {
        if (sameAsAkad?.checked) copyAkadToReception();
      }));
    ...
    read: (form, sections, old) => {
      ...
      if (sections.includes("couple")) {
        Object.assign(wedding.couple.groom, { name: trim(elements.groom_name.value), full_name: trim(elements.groom_full_name.value), photo: elements.groom_photo.value, father: trim(elements.groom_father.value), mother: trim(elements.groom_mother.value) });
        Object.assign(wedding.couple.bride, { name: trim(elements.bride_name.value), full_name: trim(elements.bride_full_name.value), photo: elements.bride_photo.value, father: trim(elements.bride_father.value), mother: trim(elements.bride_mother.value) });
      }
      if (sections.includes("event")) {
        wedding.event.akad = readEvent(elements, "akad", wedding.event.akad);
        wedding.event.reception = readEvent(elements, "reception", wedding.event.reception);
      ...
      if (sections.includes("digital_gift")) {
        wedding.digital_gift = {
          ...
          accounts: [...form.querySelectorAll(".account-row")].map((row) => ({
      ...
      if (sections.includes("music")) wedding.music = { enabled: elements.music_enabled.checked, url: trim(elements.music_url.value), autoplay: elements.music_autoplay.checked };
      return wedding;
    },
    ...
    validate: (form, sections) => {
      const elements = form.elements;
      if (sections.includes("couple")) {
        if (!trim(elements.groom_name.value)) return "Nama panggilan pengantin pria wajib diisi.";
        if (!trim(elements.bride_name.value)) return "Nama panggilan pengantin wanita wajib diisi.";
      }
      if (sections.includes("event")) {
        ...
        for (const [key, label, enabled] of [["akad", "Akad", akad], ["reception", "Resepsi", reception]]) {
          if (enabled) {
            if (!elements[`${key}_date`].value) return `Tanggal ${label} wajib diisi.`;
            if (!elements[`${key}_start_time`].value) return `Waktu mulai ${label} wajib diisi.`;
            if (!trim(elements[`${key}_location`].value)) return `Lokasi ${label} wajib diisi.`;
          }
        }
        if (!elements.countdown_target.value || !elements[`${elements.countdown_target.value}_enabled`]?.checked) return "Countdown harus mengarah ke acara yang aktif.";
      }
      if (sections.includes("digital_gift") && elements.gift_enabled.checked) {
        const rows = [...form.querySelectorAll(".account-row")];
        if (!rows.length) return "Digital Gift aktif membutuhkan minimal satu rekening.";
        for (const row of rows) {
          if (!trim(row.querySelector('[name="account_bank"]').value) || !trim(row.querySelector('[name="account_number"]').value) || !trim(row.querySelector('[name="account_holder"]').value)) return "Lengkapi Bank, Nomor Rekening, dan Atas Nama pada rekening Digital Gift.";
        }
      }
      if (sections.includes("music") && elements.music_enabled.checked && !trim(elements.music_url.value)) return "Music URL wajib diisi ketika Musik diaktifkan.";
      return "";
    }
  };
})();
